...
class Google_OAuth2 extends Google_Auth {
  ...
  public function __construct() {
    global $gdwpm_apiConfig;

    if (! empty($gdwpm_apiConfig['developer_key'])) {
      $this->developerKey = $gdwpm_apiConfig['developer_key'];
    }

    if (! empty($gdwpm_apiConfig['oauth2_client_id'])) {
      $this->clientId = $gdwpm_apiConfig['oauth2_client_id'];
    }

    if (! empty($gdwpm_apiConfig['oauth2_client_secret'])) {
      $this->clientSecret = $gdwpm_apiConfig['oauth2_client_secret'];
    }

    if (! empty($gdwpm_apiConfig['oauth2_redirect_uri'])) {
      $this->redirectUri = $gdwpm_apiConfig['oauth2_redirect_uri'];
    }

    if (! empty($gdwpm_apiConfig['oauth2_access_type'])) {
      $this->accessType = $gdwpm_apiConfig['oauth2_access_type'];
    }

    if (! empty($gdwpm_apiConfig['oauth2_approval_prompt'])) {
      $this->approvalPrompt = $gdwpm_apiConfig['oauth2_approval_prompt'];
    }
